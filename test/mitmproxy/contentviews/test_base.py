...
class TestContentView2(contentviews.View):
    name = "test2"
    prompt = ("t", "test")
    content_types = ["test/123"]
...
def test_add_remove():
    ...
    with pytest.raises(ContentViewException):
        contentviews.add(TestContentView2())
...
def test_format_dict():
    ...
    r = list(contentviews.format_dict(d))
    assert len(r) == 2
    assert [('header', 'foo: '), ('text', 'bar')] in r
    assert [('header', b'bla: '), ('text', b'fasel')] in r
...
def test_safe_to_print():
    ...
    r = contentviews.safe_to_print(lines)
    assert list(r)

...
class TestLayer:
    def test_debug_messages(self, tctx):
        ...
        assert (
            ...
            << commands.Log(
                " << OpenConnection({'connection': Server({'id': 'serverid', 'address': None})})",
                "debug",
            )
            ...
            << commands.Log(
                ...
                "{'id': 'serverid', 'address': None, 'state': <ConnectionState.OPEN: 3>})}))",
                "debug",
            )
            << commands.Log(" !> DataReceived(client, b'foo')", "debug")
...
class TestNextLayer:
    ...
    def test_receive_close(self, tctx, layer_found):
        ...
        if layer_found:
            ...
            assert (
                playbook >> tutils.reply(to=-2) << commands.CloseConnection(tctx.client)
            )
    ...
    def test_func_references(self, tctx):
        ...
        assert playbook >> tutils.reply() << commands.SendData(tctx.client, b"foo")

...
async def test_generated_files(filename):
    ...
    elif filename.stem == "generate_columns":
        mod.generate_columns()
        expected_file = Path("mitmproxy/tools/web/web_columns.py")
        assert expected_file.exists(), f"{expected_file} was not generated."
        content = expected_file.read_text()
        assert "WEB_COLUMNS" in content, "WEB_COLUMNS not found in generated file"
        assert (
            "DEFAULT_COLUMNS" in content
        ), "DEFAULT_COLUMNS not found in generated file"
    else:
        pytest.fail(f"Unexpected file {filename} without 'make' function")

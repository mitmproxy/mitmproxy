...
from web.gen import options_js
...
@pytest.mark.parametrize(
        "filename", 
        list((here / "../../../../web/gen").glob("*.py"))
)
async def test_generated_files(filename):
    mod = importlib.import_module(f"web.gen.{filename.stem}")
    expected = await mod.make()
    assert mod.filename.read_bytes() == expected.encode(), f"{mod.filename} must be regenerated by running {filename.resolve()}."

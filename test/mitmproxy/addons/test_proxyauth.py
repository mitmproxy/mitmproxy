...
class TestProxyAuth:
    ...
    def test_which_auth_header(self, is_proxy_auth, expected):
        up = proxyauth.ProxyAuth()
        with mock.patch(
            "mitmproxy.addons.proxyauth.ProxyAuth.is_proxy_auth",
            return_value=is_proxy_auth,
        ):
            assert up.which_auth_header() == expected

    @pytest.mark.parametrize(
        "is_proxy_auth, expected_status_code, expected_header",
        [
    ...
    def test_auth_required_response(
        self, is_proxy_auth, expected_status_code, expected_header
    ):
        up = proxyauth.ProxyAuth()
        with mock.patch(
            "mitmproxy.addons.proxyauth.ProxyAuth.is_proxy_auth",
    ...
    def test_check(self, tdata):
        ...
        with taddons.context(up) as ctx:
            ...
            ctx.configure(up, proxyauth="@" + tdata.path("mitmproxy/net/data/htpasswd"))
            f.request.headers["Proxy-Authorization"] = proxyauth.mkauth("test", "test")
            assert up.check(f)
            f.request.headers["Proxy-Authorization"] = proxyauth.mkauth("test", "foo")
            assert not up.check(f)

            with mock.patch(
                "ldap3.Server", return_value="ldap://fake_server:389 - cleartext"
            ):
    ...
    def test_configure(self, tdata):
        ...
        with taddons.context(up) as ctx:
            ...
            with pytest.raises(exceptions.OptionsError):
                ctx.configure(
                    up, proxyauth="ldap:fake_serveruid=?dc=example,dc=com:person"
                )
            ...
            with pytest.raises(exceptions.OptionsError):
                ctx.configure(
                    up, proxyauth="@" + tdata.path("mitmproxy/net/data/server.crt")
                )
            with pytest.raises(exceptions.OptionsError):

...
class TestStripECH:
    def test_simple(self):
        ...
        with taddons.context(se) as tctx:
            params1 = {SVCParamKeys.PORT.value: b"\x01\xbb", SVCParamKeys.ECH.value: b"testbytes"}
            ...
            answers = [
                ...
                dns.ResourceRecord(
                    "dns.google", dns.types.HTTPS, dns.classes.IN, 32, https_records.pack(record1)
                ),
                dns.ResourceRecord(
                    "dns.google", dns.types.HTTPS, dns.classes.IN, 32, https_records.pack(record2)
                ),
            ]
            ...
            se.dns_response(f)
            assert all(answer.https_ech is None for answer in f.response.answers if answer.type == types.HTTPS)

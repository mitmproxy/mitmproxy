...
class DummyLoop:
    ...
    async def getaddrinfo(self, host: str, port: int, *, family: int):
        ...
        if family == socket.AF_INET:
            ...
            if host == "dns.google":
                return [(socket.AF_INET6, None, None, None, ("2001:4860:4860::8888", port, None, None))]
        else:
...
async def test_resolve():
    ...
    async def succeed_with(question: dns.Question, check: Callable[[dns.ResourceRecord], bool]):
        assert any(map(check, await dns_resolver.resolve_question(question, DummyLoop())))
    ...
    req = tutils.tdnsreq()
    req.query = False
    assert (await dns_resolver.resolve_message(req, DummyLoop())).response_code == dns.response_codes.REFUSED
    req.query = True
    req.op_code = dns.op_codes.IQUERY
    assert (await dns_resolver.resolve_message(req, DummyLoop())).response_code == dns.response_codes.NOTIMP

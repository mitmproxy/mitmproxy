...
def test_unpack_from_with_compression():
    ...
    assert domain_names.unpack_from_with_compression(
        b'\xFF\xFF\xFF\x07example\x03org\x00\xFF\xFF\xFF\x03www\xc0\x03', 19, domain_names.cache()
    ) == ("www.example.org", 6)
...
def test_pack():
    ...
    with pytest.raises(ValueError, match=re.escape("encoding with 'idna' codec failed (UnicodeError: label too long)")):

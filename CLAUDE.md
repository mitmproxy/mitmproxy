...
Before building, run the unit tests to verify your changes don't break existing functionality:

```bash
# Run all addon tests (271 tests)
python -m pytest mitmproxy/addons/oximy/tests/ -v

# Run with coverage report
python -m pytest mitmproxy/addons/oximy/tests/ --cov=mitmproxy.addons.oximy --cov-report=term-missing

# Run specific test file
python -m pytest mitmproxy/addons/oximy/tests/test_normalize.py -v
python -m pytest mitmproxy/addons/oximy/tests/test_process.py -v
...
|------|-------|----------------|
| `normalize.py` | 89 | gRPC decoding, SSE parsing, anti-hijack handling, msgpack, base64, compression |
| `process.py` | 78 | Process resolution, PID lookup, bundle ID extraction, caching, async operations |
| `addon.py` | 86 | Domain matching, whitelist/blacklist, GraphQL extraction, memory buffer, TLS passthrough |

**Claude Note:** Always run tests after modifying addon code:
```bash
python -m pytest mitmproxy/addons/oximy/tests/ -v --tb=short
```
Expected output: `271 passed`

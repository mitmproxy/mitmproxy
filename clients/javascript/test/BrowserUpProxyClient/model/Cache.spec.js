/**
 * BrowserUp Proxy
 * BrowserUp Proxy Control API
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/BrowserUpProxyClient/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/BrowserUpProxyClient/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.BrowserUpProxyClient);
  }
}(this, function(expect, BrowserUpProxyClient) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new BrowserUpProxyClient.Cache();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('Cache', function() {
    it('should create an instance of Cache', function() {
      // uncomment below and update the code to test Cache
      //var instane = new BrowserUpProxyClient.Cache();
      //expect(instance).to.be.a(BrowserUpProxyClient.Cache);
    });

    it('should have the property beforeRequest (base name: "beforeRequest")', function() {
      // uncomment below and update the code to test the property beforeRequest
      //var instance = new BrowserUpProxyClient.Cache();
      //expect(instance).to.be();
    });

    it('should have the property afterRequest (base name: "afterRequest")', function() {
      // uncomment below and update the code to test the property afterRequest
      //var instance = new BrowserUpProxyClient.Cache();
      //expect(instance).to.be();
    });

    it('should have the property comment (base name: "comment")', function() {
      // uncomment below and update the code to test the property comment
      //var instance = new BrowserUpProxyClient.Cache();
      //expect(instance).to.be();
    });

  });

}));

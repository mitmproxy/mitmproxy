...
class EventCode(Enum):
    ...
    # Trace
    TRACE_CAPTURE_001 = ("TRACE.CAPTURE.001", LogLevel.INFO, ActionCategory.NONE)
    TRACE_WRITE_001 = ("TRACE.WRITE.001", LogLevel.INFO, ActionCategory.NONE)
...
class _OximyLogger:
    ...
    def __init__(self) -> None:
        ...
        self._sentry_counts: dict[str, tuple[int, float]] = {}  # code -> (count, window_start)
    ...
    def _send_to_sentry(self, code: EventCode, msg: str, data: dict | None,
        ...
        # Capture event for info+ (rate-limited per event code)
        if code.level in (LogLevel.INFO, LogLevel.WARNING, LogLevel.ERROR, LogLevel.FATAL):
            if not self._should_send_sentry_event(code.code):
                return

...
class BrowserDataAddOn:
    ...
    def _is_full_html_page(self, f: mitmproxy.http.HTTPFlow):
        ...
        if "content-type" in f.response.headers and "text/html" not in f.response.headers["content-type"]:
            logging.info("Evaluating injection for response false for content type")
            return False

        if "content-length" in f.response.headers and int(f.response.headers["content-length"]) < 100:
    ...
    def _compute_websocket_accept_value(self, key: str) -> str:
        ...
        accept_value = base64.b64encode(hashlib.sha1(combined.encode()).digest()).decode('utf-8')
        return accept_value

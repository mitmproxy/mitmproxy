...
class HarVerifications:
    ...
    def entries(self, criteria=False):
        ...
        har_entry_filters = {
            ...
            "status": (
                lambda item, status: self.rmatch(
                    str(glom(item, "response.status", default=None)), status
                )
            ),
            "url": (
                lambda item, url_rxp: self.rmatch(
                    str(glom(item, "request.url", default=None)), url_rxp
                )
            ),
            "content": (
                lambda item, content_rxp: self.rmatch(
                    str(glom(item, "response.content.text", default=None)), content_rxp
                )
            ),
            "content_type": (
                lambda item, content_type_rxp: self.rmatch(
                    str(glom(item, "response.content.mimeType", default=None)),
                    content_type_rxp,
                )
            ),
            "request_header": (
                lambda item, match_rgxp: self.rmatch_key_val(
                    glom(item, "request.headers", default=[]), match_rgxp
                )
            ),
            "response_header": (
                lambda item, match_rgxp: self.rmatch_key_val(
                    glom(item, "response.headers", default=[]), match_rgxp
                )
            ),
            "request_cookie": (
                lambda item, match_rgxp: self.rmatch_key_val(
                    glom(item, "request.cookies", default=[]), match_rgxp
                )
            ),
            "response_cookie": (
                lambda item, match_rgxp: self.rmatch_key_val(
                    glom(item, "response.cookies", default=[]), match_rgxp
                )
            ),
            "websocket_message": (
                lambda item, ws_rxp: self.rmatch_any(
                    glom(item, ("_webSocketMessages", ["data"]), default=[]), ws_rxp
                )
            ),
            "json_valid": (
                lambda item, _: self.valid_json(
                    str(glom(item, "response.content.text", default=None))
                )
            ),
            "json_path": (
                lambda item, path: self.has_json_path(
                    str(glom(item, "response.content.text", default=None)), path
                )
            ),
            "json_schema": (
                lambda item, schema: self.schema_validate(
                    str(glom(item, "response.content.text", default=None)), schema
                )
            ),
        }
        ...
        for filter_name, target_value in criteria.items():
            ...
            if filter_lambda is not None:
                ...
                entry_list = [
                    entry for entry in entry_list if filter_lambda(entry, target_value)
                ]
    ...
    def measure(self, items, measurement):
        measurements = {
            ...
            "request": (
                lambda item: self.gsize(item, "request.bodySize")
                + self.gsize(item, "request.headerSize")
            ),
            "response": (
                lambda item: self.gsize(item, "response.bodySize")
                + self.gsize(item, "response.headerSize")
            ),

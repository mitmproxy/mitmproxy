...
class Intercept:
    ...
    def load(self, loader):
        ...
        loader.add_option(
            "intercept", typing.Optional[str], None, "Intercept filter expression."
        )
            ...
            if ctx.options.intercept:
                ...
                if not self.filt:
                    raise exceptions.OptionsError(
                        f"Invalid interception filter: {ctx.options.intercept}"
                    )
    ...
    def process_flow(self, f: flow.Flow) -> None:
        if self.should_intercept(f):
            ...
            if f.reply.state != "start":
                return ctx.log.debug(
                    "Cannot intercept request that is already taken by another addon."
                )
            f.intercept()

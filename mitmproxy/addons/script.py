...
class Script:
    ...
    def loadscript(self):
        ...
        if self.ns:
            ...
            try:
                ctx.master.addons.invoke_addon(self.ns, "configure", ctx.options.keys())
...
class ScriptLoader:
    ...
    def load(self, loader):
        loader.add_option("scripts", typing.Sequence[str], [], "Execute a script.")
    ...
    def script_run(self, flows: typing.Sequence[flow.Flow], path: mtypes.Path) -> None:
        ...
        if mod:
            with addonmanager.safecall():
                ...
                ctx.master.addons.invoke_addon(mod, "configure", ctx.options.keys())
                for f in flows:

...
class Save:
    ...
    def load(self, loader):
        loader.add_option(
            ...
            "Stream flows to file as they arrive. Prefix path with + to append. The full path can use python strftime() formating, missing directories are created as needed."
        )
        loader.add_option(
            "save_stream_period", typing.Optional[str], None,
            "Save to a new file every 'hour' or 'day'. Requires 'save_stream_file' to use proper formating for a unique file for every hour or day. Flows are placed in files according to the end time."
    ...
    def new_stream(self):
        if self.new_path():
            ...
            try:
                ...
                ctx.log.error(f"Failed to open {self.lastpath} for writing, not saving flows anymore")
    ...
    def configure(self, updated):
        ...
        if "save_stream_period" in updated:
            if ctx.options.save_stream_period == 'hour':
                ...
                self.period = 24 * 3600
            elif ctx.options.save_stream_period == 'none':
                self.period = 0
            else:
                raise exceptions.OptionsError("save_stream_file_period must be 'none', 'hour' or 'day'.")

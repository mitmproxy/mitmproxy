...
def is_mqtt_query(data):
    return isinstance(data, dict) and "query" in data and "\n" in data["query"]


def is_mqtt_batch_query(data):
...
class ViewMQTT(base.View):
    ...
    def __call__(self, data, **metadata):
        ...
        if data is not PARSE_ERROR:
            if is_mqtt_query(data):
                return "MQTT", base.format_text(format_graphql(data))
            elif is_mqtt_batch_query(data):
                return "MQTT", base.format_text(format_query_list(data))

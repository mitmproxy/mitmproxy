...
def unpack_params(data: bytes, offset: int) -> dict:
    ...
    while offset < len(data):
        ...
        if param_type == 0:  # Mandatory
            mandatory_types = [struct.unpack('!H', param_value[i:i + 2])[0] for i in range(0, param_length, 2)]
            ...
            ipv4_addresses = [str(ipaddress.IPv4Address(param_value[i:i + 4])) for i in range(0, param_length, 4)]
            ...
            ipv6_addresses = [str(ipaddress.IPv6Address(param_value[i:i + 16])) for i in range(0, param_length, 16)]

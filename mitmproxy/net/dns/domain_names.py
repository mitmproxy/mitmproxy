...
def record_data_can_have_compression(record_type: int) -> bool:
    if record_type in (types.CNAME, types.HINFO, types.MB, types.MD, types.MF, types.MG, types.MINFO, types.MR, types.MX, types.NS, types.PTR, types.SOA, types.TXT, types.RP, types.AFSDB, types.RT, types.SIG, types.PX, types.NXT, types.NAPTR, types.SRV):
        return True
    return False


def decompress_record_data(buffer: bytes, offset: int, end_data: int, cached_names: Cache) -> bytes:

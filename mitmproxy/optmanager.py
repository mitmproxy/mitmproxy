...
class OptManager:
    ...
    def _parse_setval(self, o: _Option, values: list[str]) -> Any:
        ...
        if o.typespec in (str, Optional[str]):
            ...
            if optstr:
                try:
                    ...
                    raise exceptions.OptionsError(f"Failed to parse option {o.name}: not an integer: {optstr}")
        ...
        raise NotImplementedError(f"Failed to parse option {o.name}: unsupported option type: {o.typespec}")

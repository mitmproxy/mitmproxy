...
class ServerInstance(Generic[M], metaclass=ABCMeta):
    ...
    async def handle_stream(
        ...
        elif isinstance(self.mode, (mode_specs.WireGuardMode, mode_specs.LocalMode)):  # pragma: no cover on platforms without wg-test-client
...
class WireGuardServerInstance(ServerInstance[mode_specs.WireGuardMode]):
    ...
    def make_top_layer(self, context: Context) -> Layer:  # pragma: no cover on platforms without wg-test-client
    ...
    async def wg_handle_stream(self, stream: mitmproxy_rs.Stream) -> None:  # pragma: no cover on platforms without wg-test-client
        await self.handle_stream(stream, stream)

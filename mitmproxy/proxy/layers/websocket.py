...
class WebsocketLayer(layer.Layer):
    ...
    def __init__(self, context: Context, handshake_flow: http.HTTPFlow):
        ...
        self.flow = websocket.WebSocketFlow(
            context.client, context.server, handshake_flow
        )
        ...
        for ws_event in src_ws.events():
            if isinstance(ws_event, wsproto.events.Message):
                ...
                if ws_event.message_finished:
                    ...
                    assert (
                        not message.killed
                    )  # this is deprecated, instead we should have .content set to emptystr.
    ...
    def __call__(self, content: Union[str, bytes]) -> Iterator[wsproto.events.Message]:
        ...
        if len(content) == sum(self.fragment_lengths):
            ...
            for fl in self.fragment_lengths[:-1]:
                yield wsproto.events.Message(
                    content[offset : offset + fl], message_finished=False
                )
                offset += fl
            ...
            while offset < total:
                yield wsproto.events.Message(
                    content[offset : offset + self.FRAGMENT_SIZE],

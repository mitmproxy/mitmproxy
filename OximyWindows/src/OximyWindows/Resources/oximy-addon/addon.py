...
def _parse_sensor_config(raw: dict, addon_instance=None) -> dict:
    ...
    # Check if the app has responded to a suggestion (used/dismissed)
    # and send feedback to the server before writing any new suggestion
    try:
        from playbooks import read_suggestion_feedback, clear_suggestion, write_suggestion_from_server
    ...
    # Write server-provided suggestion to disk for the app
    pending_suggestion = data.get("pendingSuggestion")
    if pending_suggestion:
        try:
            from playbooks import write_suggestion_from_server as _write_suggestion
